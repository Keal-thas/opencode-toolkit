...
- **`mcp-servers/mysql/`** - `mysql.test.ts`, same shape as oracle/loki: tool listing, a plain SELECT, the `START TRANSACTION READ ONLY` wrapper unconditionally rejecting an INSERT (error 1792) even from a fully-privileged account, the same wrapper *not* rejecting a CREATE TABLE (the documented DDL gap - closed by the connecting account's own grants instead, see `mcp-servers/mysql/README.md`), a nonexistent-table error, and a connection-failure regression. Needs a real MySQL instance (see `docker/docker-notes.md`'s "MySQL test instance" section).
...
- **`mcp-servers/redis/`** (the official `redis-mcp-server` PyPI package via `uvx` - not our own code, see `docs/feature-points/20-redis-mcp-server.md`) - `tests/integration/mcp-redis/redis.test.mjs` spawns the real package: tool listing, a `get` retrieving a value seeded through an independent connection, the ACL-restricted `readonlyuser` account rejecting a `set` cleanly (confirmed not to have actually written, via that same independent connection), and `get` on a missing key returning a clean message. Needs a real Redis instance (see `docker/docker-notes.md`'s "Redis test instance" section) and `uv`/`uvx` (see its "uv/uvx" section).

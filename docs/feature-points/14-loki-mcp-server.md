...
**Tested:** `mcp-servers/loki/loki.test.ts` (see `tests/README.md`) — tool listing, label/label-value discovery finding a pushed test stream, a range query finding a pushed log line by content, an empty-result query returning cleanly, and malformed LogQL returning a clean error. Runs as part of `./tests/run-all.sh` against the sandbox's real `loki` service.
